...
cdef class PythonClassName: 
  ...
  def place_singleCellBU_randomly(self, _porosity, _jump_parameter, random_seed): 
    if _porosity == "default" : _porosity = 0.5 
    if _jump_parameter == "default": _jump_parameter = 1.0 
    if random_seed == "default" : random_seed = 0 
    self.thisptr.place_singleCellBU_randomly(_porosity, _jump_parameter, random_seed) 
  ...
  def place_plane(self, _position, _extent, _jump_parameter): 
    if _jump_parameter == "default" : _jump_parameter = 1.0 
    if _extent == "default" : _extent = [ 0 ]* 5 
    if _position == "default": _position = -1 
    return self.thisptr.place_plane(_position, _extent, _jump_parameter) 
