...
// }
// template<auto nx>
// std::vector<std::array<unsigned int,nx.size()>> getConvexHull(std::vector<std::array<unsigned int, nx.size()>> _points)
// {
...
double feretDiameter_max(std::vector<unsigned int> _convexHull)
{
  ...
  unsigned int ab[4][2] = {{0,0}, {0,1}, {1,0},{1,1}};
...
template<auto nx>
static std::vector<unsigned int> getBorder(std::vector<int> _set)
{
...
struct ParticleBU : public BuildingUnit<nx>
{
  ...
  ~ParticleBU() override {}
  static const std::vector<int> getStencil(const int _random_seed, const double _feretDiameter_max, const double _feretDiameter_min)
  {
    //mimum feret durch kreis mit 2 * radius = feret
    //dann voxel dran machen, sodass max feret diameter erreicht wird.
    // dann voxel dran machen (bis area erfüllt, oder verhältnis umfang- area),  mit bedingung max_feret nicht überboten wird. 
    ...
    std::vector<unsigned int> border = CAM::getBorder<nx>(stencil);
    ...
    {
      min_feret_coord.push_back((stencil[i] / (int)direct_neigh<nx>(2 * axis_min_feret + 1) + n_fields<nx>()) % nx[axis_min_feret]);
    }
    sort( min_feret_coord.begin(), min_feret_coord.end() );
    min_feret_coord.erase( unique( min_feret_coord.begin(), min_feret_coord.end() ), min_feret_coord.end() );
    ...
    //coord_a = (_convexHull[a] / (int)direct_neigh<nx>(2 * axis_min_feret + 1) + n_fields<nx>()) % nx[axis_min_feret];
      ...
      for(unsigned int a = 0; a < border.size(); a++)
      {
        for (unsigned int i = 0; i < 2 * nx.size(); ++i)
        {
          ...
          if(std::find(stencil.begin(),stencil.end(), neigh) == stencil.end())
          {
            ...
            // //std::cout<<"max_feret "<<feretDiameter<<std::endl;
            // fields.pop_back();
            //feretDiameter < _feretDiameter_max && 
            unsigned int coord_min = (neigh / (int)direct_neigh<nx>(2 * axis_min_feret + 1) + n_fields<nx>()) % nx[axis_min_feret];
            if(find(min_feret_coord.begin(), min_feret_coord.end(),coord_min) != min_feret_coord.end())
    ...
    //   loops++;
    //   std::vector<int> newStencilPoints;
    //   for(unsigned int a = 0; a < stencil.size(); a++)
    //   {
    //     for (unsigned int i = 0; i < 2 * nx.size(); ++i)
    //     {
    //       int neigh = aim<nx>(stencil[a], direct_neigh<nx>(i));
    //       if (std::find(stencil.begin(), stencil.end(), neigh) == stencil.end())
    //       {
    //         newStencilPoints.push_back(neigh);
    //       }
    ...
    // std::array<double,2> feretDiameters = CAM::feretDiameter_min_max<nx>(fields);
    // std::cout<<"diam "<<feretDiameters[0]<<" "<<feretDiameters[1]<<std::endl;
    // std::cout<<"NrBor "<<border.size()<<std::endl;
