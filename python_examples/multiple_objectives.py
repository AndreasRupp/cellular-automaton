...
def parameter_identification_test(nx, porosity, n_steps, jump_parameter,
  ...
  def my_distance(a, b):
    avg_part_a = PyCAM.average_particle_size(a)
    avg_part_b = PyCAM.average_particle_size(b)
    return np.abs(avg_part_a - avg_part_b)

  distance_list = [ PyCAM.bulk_distance, my_distance ]
  n_bins_list   = [ 20, 8 ]
  func_list     = []
  ...
  for index in range(len(distance_list)):
    ...
    n_bins   = n_bins_list[index]
    min_val, max_val, _ = ecdf.estimate_radii_values(
      data[0:subset_sizes[0]], data[subset_sizes[0]:subset_sizes[0]+subset_sizes[1]], distance )
    bins = np.linspace(min_val, max_val, 50)

    func = ecdf.estimator(data, bins, distance, subset_sizes)
    ...
    func.choose_bins(n_bins, min_value_shift, max_value_shift)
  ...
  obj_func = ecdf.multiple_estimator( func_list )

  ax[0,0] = ecdf.plot_ecdf_vectors(obj_func, ax[0,0], 'r.')
  ax[0,0] = ecdf.plot_mean_vector(obj_func, ax[0,0], 'k.')
  ax[1,1] = ecdf.plot_chi2_test(obj_func, ax[1,1])
